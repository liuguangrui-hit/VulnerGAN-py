...
from nids_models.models import MLP_NIDS
...
def train(model, train_loader, epoch, is_end):
    ...
    #     np.save('data_record/fail_MLP.npy', fail_samples)
    #     print("success_samples size:", len(success_samples))
    #     success_samples = np.array(success_samples)
    #     np.save('data_record/success_MLP.npy', success_samples)
...
def test_result(model, test_loader):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            correct += (pred == target).sum().item()
            cnt += pred.shape[0]
            file_path = "data_record/0.1_NIDS_MLP_result.csv"
...
if __name__ == '__main__':
    ...
    while not loop_exit:
        ...
        if c == '4':
            ...
            loop_exit = True
    dataset_n_len = 50000
    model_path = "model_record/GRU_MLP/"
    model_name = 'NIDS_GRU_MLP'
    ...
    # test
        ...
        model_p = "model_record/MLP/1_" + model_name + ".pt"
        if os.path.exists(model_p):
            checkpoint = torch.load(model_p)
            net.model.load_state_dict(checkpoint['model'])
            optimizer.load_state_dict(checkpoint['optimizer'])

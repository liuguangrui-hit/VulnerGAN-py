...
from nids_models.save_model import save_model
...
def get_test_result(model, X_test, Y_test):
    ...
    y_pred = np.array(y_pred)
    y_pred = [np.round(x) for x in y_pred]
    file_path = "data_record/0.1_NIDS_LSTM_result.csv"
...
# hyper-parameter
epoch = 1
model_path = "model_record/GRU_LSTM/"
model_name = 'NIDS_GRU_LSTM'
...
if __name__ == '__main__':
    ...
    if not reuse_model and is_train:
        ...
        x_test, y_test = test_s.items, test_s.label
        x_test = x_test.reshape(x_test.shape[0], 1, x_test.shape[1])
        model_p = 'model_record/LSTM/1_' + model_name + ".hdf5"

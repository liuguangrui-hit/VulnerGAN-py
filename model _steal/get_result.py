...
if __name__ == '__main__':
    ...
    attack_name = 'Bot'
    file_path = "result_record/" + model_name + "/"
    if not os.path.exists(file_path):
        os.mkdir(file_path)

    for attack in attack_list:
        model_p = "nids_model/" + model_name + "/" + attack + ".pkl"
        if os.path.exists(model_p):
            net.load_state_dict(torch.load(model_p))
            # net = torch.load(model_p)
            # print(net)
        file_saved = file_path + attack + ".csv"
        if os.path.exists(file_saved):
            os.unlink(file_saved)
        test_dl = DataLoader(Dataset("../data/cic_2017/data_steal/data_test/" + attack + "_test.csv"), batch_size=32,
                         shuffle=False)

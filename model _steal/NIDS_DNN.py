...
if __name__ == '__main__':
    ...
    model_name = 'GRU'
    model_path = "steal_record/" + model_name + "_DNN/" + attack_name + "/"
    # start NIDS training
        ...
        for model_name in model_list:
            ...
            for attack_name in attack_list:
                ...
                if not os.path.exists(model_path):
                    # shutil.rmtree(model_path)
                    os.mkdir(model_path)
                x = "../data/cic_2017/data_steal/data_train/" + attack_name + "_train.csv"
                y = "data_record/" + model_name + "/" + attack_name + ".csv"
                x_t = "../data/cic_2017/data_steal/data_test/" + attack_name + "_test.csv"
                y_t = "result_record/" + model_name + "/" + attack_name + ".csv"
                train_dl = DataLoader(
                    Dataset_shadow(x, y), batch_size=32,
                    shuffle=False)
                test_dl = DataLoader(Dataset_shadow(x_t, y_t), batch_size=32, shuffle=False)
                print(net)
                is_end = False
                k = 0.1
                for i in range(epoch):  # 训10个epoch
                    ...
                    torch.save(state, model_path + str(i + 1) + "_" + attack_name + ".pt")
                    # k += 0.1
                state = {'model': net.model.state_dict(), 'optimizer': optimizer.state_dict(), 'end_epoch': i + 1,
                         'epoch': epoch}
                torch.save(state, "steal_record/" + model_name + "_DNN/" + attack_name + ".pt")
                print('========== 模型训练已完成 ==========\n\n')

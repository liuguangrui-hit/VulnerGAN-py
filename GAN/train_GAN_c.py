...
class GANDataset(Dataset):
    def __init__(self, input_file, target_file):
        ...
        # self.target_items = np.load(target_file)[:100]
...
def train(generator, discriminator, input_p, target_p, model_name, attack_name, k):
    ...
    runner.train(
        ...
        num_epochs=8,
        verbose=True,
        logdir="cheat_test/logs_gan/" + model_name + "/" + attack_name + "/",
    )
    g_p = "cheat_test/generator/" + model_name + "/" + attack_name + "/" + str(k) + "_generator.pt"
    d_p = "cheat_test/discriminator/" + model_name + "/" + attack_name + "/" + str(k) + "_discriminator.pt"
...
if __name__ == "__main__":
    ...
    if not reuse_model and is_train:
        ...
        # target_p = 'input_record/' + model_name + '/' + attack_name + '_success.npy'
        target_p = 'cheat_test/target_record/' + model_name + '/' + attack_name + '_fail.npy'
    ...
    # continue NIDS training
        ...
        for model_name in model_list:
            ...
            for attack_name in attack_list:
                ...
                model_d = "cheat_test/discriminator/" + model_name + "/" + attack_name + "/" + str(
                    k) + "_discriminator.pt"
                # success作为input送入生成器G fail作为target送入鉴别器D
                input_p = 'input_record/' + model_name + '/' + attack_name + '_success.npy'
                target_p = 'cheat_test/target_record/' + model_name + '/' + attack_name + '_fail_' + str(k) + '.npy'
